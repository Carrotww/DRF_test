...
class ArticleCreateTest(APITestCase):
    ...
    def setUpTestData(cls):
        cls.user_data = {'username': 'deft', 'password': 'deft'}
        cls.article_data = {'title': 'some title', 'content': 'some content'}
        cls.user = User.objects.create_user('deft', 'deft')
    
    def setUp(self):
        self.access_token = self.client.post(reverse('token_obtain_pair'), self.user_data).data['access']

    # def setUp(self) -> None:
    #     self.user_data = {'username': 'deft', 'password': 'deft'}
...
class ArticleReadTest(APITestCase):
    ...
    def setUpTestData(cls) -> None:
        ...
        for i in range(10):
            ...
            cls.articles.append(Article.objects.create(title=cls.faker.sentence(), content=cls.faker.text()))
